(function () {
  angular.module('jsCustomSelect.directives')
    .directive('customSelect', ['$parse', '$compile', '$timeout', 'jsCustomSelect.defaults', function ($parse, $compile, $timeout, baseOptions) {
      ...
      return {
        ...
        link: function (scope, elem, attrs, controller) {
          ...
          var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
            ...
            selectTemplate = '<select class="hide" ng-options="' + attrs.ngOptions + '" ng-model="' +
              attrs.ngModel + '" ' +
              (attrs.ngChange ? 'ng-change="' + attrs.ngChange + '"' : '') +
              '></select>',
            dropdownTemplate =
